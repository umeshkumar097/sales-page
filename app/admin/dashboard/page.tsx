...
function DashboardContent() {
    ...
    const [modalTab, setModalTab] = useState<"Overview" | "Notes" | "Schedule" | "Proposal">("Overview");
    ...
    const [proposalTimeline, setProposalTimeline] = useState("4-6 Weeks");
    ...
    const handleSaveNotes = async () => {
        ...
        try {
            ...
            setLeads(leads.map(lead => lead.id === selectedLead.id ? { ...lead, notes: leadNotes } : lead));
    ...
    const openModal = (lead: Lead) => {
        ...
        // Setup Proposal Defaults
        setProposalPrice(lead.budget || "");
        setProposalScope(lead.projectType || "");
        setProposalTimeline("4-6 Weeks");
        ...
        <div className={styles.dashboardContainer}>
            ...
            {isModalOpen && selectedLead && (
                <div className={styles.modalOverlay}>
                    <div className={`glass ${styles.modalContent}`} style={{ maxWidth: '600px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', borderBottom: '1px solid rgba(255,255,255,0.1)', marginBottom: '1.5rem', paddingBottom: '0.5rem' }}>
                            <button 
                                ...
                                style={{ background: 'none', border: 'none', color: modalTab === "Overview" ? 'var(--accent-primary)' : 'var(--text-secondary)', fontWeight: modalTab === "Overview" ? 600 : 400, cursor: 'pointer', padding: '0.5rem 0' }}
                            >
                                Overview
                            </button>
                            <button 
                                onClick={() => setModalTab("Notes")} 
                                style={{ background: 'none', border: 'none', color: modalTab === "Notes" ? 'var(--accent-primary)' : 'var(--text-secondary)', fontWeight: modalTab === "Notes" ? 600 : 400, cursor: 'pointer', padding: '0.5rem 0', marginLeft: '1rem' }}
                            >
                                Sales Notes
                            </button>
                            <button 
                                onClick={() => setModalTab("Schedule")} 
                                style={{ background: 'none', border: 'none', color: modalTab === "Schedule" ? 'var(--accent-primary)' : 'var(--text-secondary)', fontWeight: modalTab === "Schedule" ? 600 : 400, cursor: 'pointer', padding: '0.5rem 0', marginLeft: '1rem' }}
                            >
                                Schedule
                            </button>
                            <button 
                                onClick={() => setModalTab("Proposal")} 
                                style={{ background: 'none', border: 'none', color: modalTab === "Proposal" ? 'var(--accent-primary)' : 'var(--text-secondary)', fontWeight: modalTab === "Proposal" ? 600 : 400, cursor: 'pointer', padding: '0.5rem 0', marginLeft: '1rem' }}
                            >
                                Proposal
                            </button>
                        </div>
                        ...
                        <div className={styles.modalBody}>
                            {modalTab === "Overview" && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', background: 'rgba(255,255,255,0.02)', padding: '1.5rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                        <div>
                                            <p className="text-muted" style={{ fontSize: '0.8rem', margin: 0 }}>Company</p>
                                            <p style={{ margin: '0.25rem 0 0', fontWeight: 500 }}>{selectedLead.company || "N/A"}</p>
                                        </div>
                                        <div>
                                            <p className="text-muted" style={{ fontSize: '0.8rem', margin: 0 }}>Service</p>
                                            <p style={{ margin: '0.25rem 0 0', fontWeight: 500 }}>{selectedLead.projectType}</p>
                                        </div>
                                        <div>
                                            <p className="text-muted" style={{ fontSize: '0.8rem', margin: 0 }}>Budget</p>
                                            <p style={{ margin: '0.25rem 0 0', fontWeight: 500 }}>{selectedLead.budget || "N/A"}</p>
                                        </div>
                                        <div>
                                            <p className="text-muted" style={{ fontSize: '0.8rem', margin: 0 }}>Lead Score</p>
                                            <p style={{ margin: '0.25rem 0 0', fontWeight: 500, color: selectedLead.score?.includes('Hot') ? '#ef4444' : selectedLead.score?.includes('Warm') ? '#eab308' : '#3b82f6' }}>{selectedLead.score || "Unscored"}</p>
                                        </div>
                                    </div>
                                    <div>
                                        <p className="text-muted" style={{ fontSize: '0.8rem', margin: '0 0 0.5rem 0' }}>Message</p>
                                        <div style={{ background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '8px', fontSize: '0.9rem', lineHeight: 1.6 }}>
                                            {selectedLead.message || <span className="text-muted">No message provided.</span>}
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            {modalTab === "Notes" && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    <label htmlFor="notes" style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Internal Sales Notes (Not visible to client)</label>
                                    <textarea 
                                        id="notes" 
                                        rows={6} 
                                        className={styles['form-control'] || 'form-control'}
                                        style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255, 255, 255, 0.1)', color: 'var(--text-primary)', padding: '0.75rem 1rem', borderRadius: 'var(--radius-sm)', width: '100%', fontFamily: 'inherit', resize: 'vertical' }}
                                        value={leadNotes}
                                        onChange={(e) => setLeadNotes(e.target.value)}
                                        placeholder="Add context about the deal, negotiation details, specific client requests..."
                                    />
                            ...
                            {modalTab === "Schedule" && (
                                <div>
                                    <p style={{marginBottom: '1rem', color: 'var(--text-secondary)', fontSize: '0.9rem'}}>
                                        Sending invite to <strong>{selectedLead.email}</strong>
                                    </p>
                                    ...
                                    <div className={styles.inputGroup} style={{marginTop: '1.5rem'}}>
                                        ...
                                        <input 
                                            ...
                                            id="meetingTime" 
                                            className={styles['form-control'] || 'form-control'}
                                            style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255, 255, 255, 0.1)', color: 'var(--text-primary)', padding: '0.75rem 1rem', borderRadius: 'var(--radius-sm)', width: '100%', fontFamily: 'inherit' }}
                                            value={meetingTime}
                            ...
                            {modalTab === "Proposal" && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                                        Generate a formal PDF proposal for <strong>{selectedLead.company || selectedLead.name}</strong>.
                                    </p>
                                    <div className={styles.inputGroup}>
                                        <label htmlFor="proposalScope">Project Scope / Title</label>
                                        <input 
                                            type="text" 
                                            id="proposalScope" 
                                            className={styles['form-control'] || 'form-control'}
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                        <div className={styles.inputGroup}>
                                            <label htmlFor="proposalPrice">Proposed Price (₹)</label>
                                            <input 
                                                type="text" 
                                                id="proposalPrice" 
                                                className={styles['form-control'] || 'form-control'}
                                        ...
                                        <div className={styles.inputGroup}>
                                            <label htmlFor="proposalTimeline">Estimated Timeline</label>
                                            <input 
                                                type="text" 
                                                id="proposalTimeline" 
                                                className={styles['form-control'] || 'form-control'}
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem' }}>
                                        <button 
                                            onClick={() => window.open(`/admin/proposal/${selectedLead.id}?price=${encodeURIComponent(proposalPrice)}&timeline=${encodeURIComponent(proposalTimeline)}&scope=${encodeURIComponent(proposalScope)}`, '_blank')}
                                            className="btn btn-primary"
                                            style={{ padding: '0.4rem 1rem', fontSize: '0.85rem', background: '#db2777', borderColor: '#be185d' }}
                                        >
                                            📄 Generate & Print PDF
                                        </button>
                                    </div>
            ...
            <main className={styles.main}>
                <div className={styles.statsRow}>
                    ...
                    <div className={`glass ${styles.statCard}`}>
                        <h3>Leads Today</h3>
                        <p style={{ color: '#2dd4bf' }}>{leads.filter(l => {
                    ...
                    <div className={`glass ${styles.statCard}`}>
                        ...
                        <p style={{ color: '#fbbf24' }}>{leads.length > 0 ? Math.round((leads.filter(l => l.status === "Won").length / leads.length) * 100) : 0}%</p>
                    </div>
                    <div className={`glass ${styles.statCard}`}>
                        <h3>Est. Pipeline Value</h3>
                        <p style={{ color: '#4ade80' }}>
                            {new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(
                                leads.filter(l => l.status !== "Lost").reduce((sum, l) => {
                                    if (l.budget) {
                                        if (l.budget.includes("5L+")) return sum + 500000;
                                        if (l.budget.includes("1L")) return sum + 100000;
                                        if (l.budget.includes("50K")) return sum + 50000;
                                        if (l.budget.includes("25K")) return sum + 25000;
                                    }
                                    return sum + 0;
                                }, 0)
                            )}
                                        ...
                                        <tr key={lead.id}>
                                            ...
                                            <td>
                                                <div style={{ fontSize: '0.85rem' }}>
                                                    <strong>Source:</strong> {lead.source || "Website Organic"}
                                                    <br />
                                                    <strong>Budget:</strong> {lead.budget || <span className="text-muted">N/A</span>}
                                                    <br />
                                                    {lead.score && <span style={{ display: 'inline-block', marginTop: '0.25rem', padding: '0.1rem 0.5rem', background: 'rgba(239,68,68,0.1)', color: '#ef4444', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 600 }}>{lead.score}</span>}
                                                </div>
                                            </td>
                                            <td>
                                                ...
                                                {lead.message && (
                                                    <div style={{ marginTop: '0.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)', maxWidth: '200px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={lead.message}>
                                                        "{lead.message}"
                                                    </div>
                                                )}
                                            </td>
                                            ...
                                            <td style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-start' }}>
                                                {lead.meetings && lead.meetings.length > 0 ? (
                                                    ...
                                                    <button 
                                                        ...
                                                        className="btn btn-primary"
                                                        style={{ padding: "0.4rem 0.8rem", fontSize: "0.8rem", width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.2)", color: "var(--text-primary)" }}
                                                    >
                                                        Review Deal
                                                    </button>
                                                )}
